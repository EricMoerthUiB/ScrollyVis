...
export function getInstances(camera){
    ...
    instances[9] = (p) => {
        ...
        if (p === 1 && !once[9]) {
            factors[9] = factor;
            predecessor[1] = predecessor[9] + factor /23;
            listenerRemover[1] = uos(predecessor[9] , predecessor[9] + factor /23, p => instances[1](p));
            once[9] = true;
        }
    };
    once[1] = false;
    instances[1] = (p) => {
        ...
        if (p === 1 && !once[1]) {
            factors[1] = factor;
            predecessor[3] = predecessor[1] + factor /23;
            listenerRemover[3] = uos(predecessor[1] , predecessor[1] + factor /23, p => instances[3](p));
            once[1] = true;
        }
    };
    once[3] = false;
    instances[3] = (p) => {
        ...
        if (p === 1 && !once[3]) {
            factors[3] = factor;
            predecessor[2] = predecessor[3] + factor /23;
            listenerRemover[2] = uos(predecessor[3] , predecessor[3] + factor /23, p => instances[2](p));
            once[3] = true;
        }
    };
    once[2] = false;
    instances[2] = (p) => {
        ...
        if (p === 1 && !once[2]) {
            factors[2] = factor;
            predecessor[4] = predecessor[2] + factor /23;
            listenerRemover[4] = uos(predecessor[2] , predecessor[2] + factor /23, p => instances[4](p));
            once[2] = true;
        }
    };
    once[4] = false;
    instances[4] = (p) => {
        if (p <= 0) {
            ...
            document.getElementById("6").style.zIndex = "5";
            document.getElementById("6").style.opacity = ramp(p);
        }
        if (p === 1 && !once[4]) {
            factors[4] = factor;
            predecessor[5] = predecessor[4] + factor /23;
            listenerRemover[5] = uos(predecessor[4] , predecessor[4] + factor /23, p => instances[5](p));
            once[4] = true;
        }
    };
    once[5] = false;
    instances[5] = (p) => {
        if (p <= 0) {
            ...
            if (p< 1) SKETCHFAB.sketch7Behaviour(p);
        }
        if (p === 1 && !once[5]) {
            factors[5] = factor;
            predecessor[6] = predecessor[5] + factor /23;
            listenerRemover[6] = uos(predecessor[5] , predecessor[5] + factor /23, p => instances[6](p));
            once[5] = true;
        }
    };
    once[6] = false;
    instances[6] = (p) => {
        if (p <= 0) {
            ...
            document.getElementById("8").style.zIndex = "7";
            document.getElementById("8").style.opacity = ramp(p);
        }
        if (p === 1 && !once[6]) {
            factors[6] = factor;
            predecessor[7] = predecessor[6] + factor /23;
            listenerRemover[7] = uos(predecessor[6] , predecessor[6] + factor /23, p => instances[7](p));
            once[6] = true;
        }
    };
    once[7] = false;
    instances[7] = (p) => {
        if (p <= 0) {
            ...
            if (p< 1) SKETCHFAB.sketch9Behaviour(p);
        }
        if (p === 1 && !once[7]) {
            factors[7] = factor;
            predecessor[8] = predecessor[7] + factor /23;
            listenerRemover[8] = uos(predecessor[7] , predecessor[7] + factor /23, p => instances[8](p));
            once[7] = true;
        }
    };
    once[8] = false;
    instances[8] = (p) => {
        if (p <= 0) {
            ...
            document.getElementById("10").style.zIndex = "9";
            document.getElementById("10").style.opacity = ramp(p);
        }
        if (p === 1 && !once[8]) {
            factors[8] = factor;
            predecessor[10] = predecessor[8] + factor /23;
            listenerRemover[10] = uos(predecessor[8] , predecessor[8] + factor /23, p => instances[10](p));
            once[8] = true;
        }
    };
    once[10] = false;
    instances[10] = (p) => {
        if (p <= 0) {
            ...
            if (p< 1) SKETCHFAB.sketch12Behaviour(p);
        }
        if (p === 1 && !once[10]) {
            factors[10] = factor;
            predecessor[11] = predecessor[10] + factor /23;
            listenerRemover[11] = uos(predecessor[10] , predecessor[10] + factor /23, p => instances[11](p));
            once[10] = true;
        }
    };
    once[11] = false;
    instances[11] = (p) => {
        ...
        if (p === 1 && !once[11]) {
            factors[11] = factor;
            predecessor[12] = predecessor[11] + factor /23;
            listenerRemover[12] = uos(predecessor[11] , predecessor[11] + factor /23, p => instances[12](p));
            once[11] = true;
        }
    };
    once[12] = false;
    instances[12] = (p) => {
        ...
        if (p === 1 && !once[12]) {
            factors[12] = factor;
            predecessor[13] = predecessor[12] + factor /23;
            listenerRemover[13] = uos(predecessor[12] , predecessor[12] + factor /23, p => instances[13](p));
            once[12] = true;
        }
    };
    once[13] = false;
    instances[13] = (p) => {
        ...
        if (p === 1 && !once[13]) {
            factors[13] = factor;
            predecessor[14] = predecessor[13] + factor /23;
            listenerRemover[14] = uos(predecessor[13] , predecessor[13] + factor /23, p => instances[14](p));
            once[13] = true;
        }
    };
    once[14] = false;
    instances[14] = (p) => {
        ...
        if (p === 1 && !once[14]) {
            factors[14] = factor;
            predecessor[15] = predecessor[14] + factor /23;
            listenerRemover[15] = uos(predecessor[14] , predecessor[14] + factor /23, p => instances[15](p));
            once[14] = true;
        }
    };
    once[15] = false;
    instances[15] = (p) => {
        ...
        if (p === 1 && !once[15]) {
            factors[15] = factor;
            predecessor[16] = predecessor[15] + factor /23;
            listenerRemover[16] = uos(predecessor[15] , predecessor[15] + factor /23, p => instances[16](p));
            once[15] = true;
        }
    };
    once[16] = false;
    instances[16] = (p) => {
        ...
        if (p === 1 && !once[16]) {
            factors[16] = factor;
            predecessor[17] = predecessor[16] + factor /23;
            listenerRemover[17] = uos(predecessor[16] , predecessor[16] + factor /23, p => instances[17](p));
            once[16] = true;
        }
    };
    once[17] = false;
    instances[17] = (p) => {
        ...
        if (p === 1 && !once[17]) {
            factors[17] = factor;
            predecessor[18] = predecessor[17] + factor /23;
            listenerRemover[18] = uos(predecessor[17] , predecessor[17] + factor /23, p => instances[18](p));
            once[17] = true;
        }
    };
    once[18] = false;
    instances[18] = (p) => {
        ...
        if (p === 1 && !once[18]) {
            factors[18] = factor;
            predecessor[19] = predecessor[18] + factor /23;
            listenerRemover[19] = uos(predecessor[18] , predecessor[18] + factor /23, p => instances[19](p));
            once[18] = true;
        }
    };
    once[19] = false;
    instances[19] = (p) => {
        if (p <= 0) {
            ...
            if (p< 1) SKETCHFAB.sketch22Behaviour(p);
        }
        if (p === 1 && !once[19]) {
            factors[19] = factor;
            predecessor[21] = predecessor[19] + factor /23;
            listenerRemover[21] = uos(predecessor[19] , predecessor[19] + factor /23, p => instances[21](p));
            once[19] = true;
        }
    };
    once[21] = false;
    instances[21] = (p) => {
        if (p <= 0) {
            ...
            document.getElementById("24").style.zIndex = "20";
            document.getElementById("24").style.opacity = ramp(p);
        }
        if (p === 1 && !once[21]) {
            factors[21] = factor;
            predecessor[20] = predecessor[21] + factor /23;
            listenerRemover[20] = uos(predecessor[21] , predecessor[21] + factor /23, p => instances[20](p));
            once[21] = true;
        }
    };
    once[20] = false;
    instances[20] = (p) => {
        ...
        if (p === 1 && !once[20]) {
            factors[20] = factor;
            predecessor[22] = predecessor[20] + factor /23;
            listenerRemover[22] = uos(predecessor[20] , predecessor[20] + factor /23, p => instances[22](p));
            once[20] = true;
        }
    };
    once[22] = false;
    instances[22] = (p) => {
        ...
        return [start, instances,once,23];
}
